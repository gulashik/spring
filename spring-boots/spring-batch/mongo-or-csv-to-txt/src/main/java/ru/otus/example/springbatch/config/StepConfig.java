...
public class StepConfig {
    ...
    public MongoPagingItemReader<Person> readerMongo(MongoTemplate template) {
        ...
        return new MongoPagingItemReaderBuilder<Person>()
            .name("personItemReader") // todo имя ItemReader
            .template(template) // todo кто нам достаёт
            .jsonQuery("{}")  // todo что заправшиваем
            .targetType(Person.class) // todo во что конвертируем
            .pageSize(10)
            .sorts(sortMap)
            .build();
    }
    ...
    public FlatFileItemReader<Person> readerFlatFile(@Value("#{jobParameters['" + StepConfig.INPUT_FILE_NAME + "']}") String inputFileName) {
        return new FlatFileItemReaderBuilder<Person>()
            .name("personItemReader") // todo имя ItemReader
            .resource(new FileSystemResource(inputFileName)) // todo Откуда берём путь
            .delimited()
            .names("name", "age")
            .fieldSetMapper(
    ...
    public ItemProcessor<Person, Person> processor(HappyBirthdayService happyBirthdayService) {
        return happyBirthdayService::doHappyBirthday; // todo логика модификации
    }
    ...
    // Создание Tasklet Step
    @Bean
    public Step cleanUpStep() {
        return new StepBuilder("cleanUpStep", jobRepository)
            .tasklet(
                cleanUpTasklet(), // todo Adapter для Tasklet
                platformTransactionManager
            )
            .build();
    }

    // todo Собираем полный Step с обработчиками=================
    @Bean
    public Step transformPersonsStep(
        ItemReader<Person> reader, // читает
        ItemProcessor<Person, Person> itemProcessor, // обрабатывает
        FlatFileItemWriter<Person> writer // записывает
    ) {
