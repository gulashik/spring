 ...
 *
 * Аннотация @RepositoryRestController указывает, что этот контроллер будет использовать
 * базовый путь Spring Data REST, но не будет автоматически добавлять ссылки HATEOAS.
 *
 * Аннотация @BasePathAwareController указывает, что контроллер осведомлен о базовом пути API
 * и будет использовать его для своих эндпоинтов.
 */
