 ...
 *
 * <h2>Назначение</h2>
 * Описывает все параметры, нужные стартеру для создания одного {@link com.zaxxer.hikari.HikariDataSource}.
 * Класс является обычным <em>POJO</em>-носителем свойств — никакой логики тут нет специально:
 * чем тоньше слой конфигурации, тем легче его тестировать и переиспользовать.
 *
 * <h2>Почему record</h2>
 * Spring Boot 3 предпочитает <strong>конструкторное связывание</strong> (constructor binding) для
 * properties. {@code record} — самый лаконичный и иммутабельный способ это выразить:
 * <ul>
 *   <li>иммутабельность исключает изменение конфигурации в рантайме;</li>
 *   <li>нет setter'ов — Spring не сможет «случайно» подменить значения;</li>
 *   <li>{@code -parameters} (включён в build.gradle.kts) даёт корректное имя параметра в bytecode,
 *       без него Boot не сможет связать YAML-ключ с аргументом конструктора.</li>
 * </ul>
 *
 ...
 *   <li><strong>Пароль в коде</strong> — антипаттерн. В демо мы храним его в YAML для простоты,
 *       но в проде используйте Vault, AWS Secrets Manager и т.п.</li>
 *   <li><strong>maximumPoolSize по умолчанию = 10</strong> — для read-only справочной БД
 *       это часто избыточно; явно занижайте, чтобы экономить соединения на стороне Postgres.</li>
 ...
 * @param maximumPoolSize    максимальный размер пула Hikari. По умолчанию 5 — разумный
 *                           старт для «вспомогательной» БД; основная dwh должна иметь больше.
