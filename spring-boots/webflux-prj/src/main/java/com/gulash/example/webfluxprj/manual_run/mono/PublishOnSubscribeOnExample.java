...
public class PublishOnSubscribeOnExample {
    ...
    private static void exampleSubscribeOnPublishOn() {
        ...
        Disposable disposable = Mono.just(1)
            ...
            .publishOn(Schedulers.boundedElastic()) // todo НИЖЕ переключение на другой планировщик
            .map(i -> {
                System.out.println("Map 2 on thread(будет boundedElastic): " + Thread.currentThread().getName());
                return i * 2;
            })
            .publishOn(Schedulers.single()) // todo НИЖЕ переключение на другой планировщик
            .map(i -> {
                System.out.println("Map 3 on thread(будет single): " + Thread.currentThread().getName());
                return i * 3;
            })
            .subscribeOn(Schedulers.parallel()) // todo для ВСЕГО потока
